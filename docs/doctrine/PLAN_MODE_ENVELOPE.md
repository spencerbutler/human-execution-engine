...
PLAN MODE ENVELOPE is the control-plane doctrine governing agent behavior during PLAN phase in all HEE workflows.
It enforces disciplined planning, explicit authority transitions, and first-class no-op outcomes to prevent accidental
mutation and ensure deterministic execution.
...
The doctrine enables tool-agnostic enforcement through structural constraints, ensuring consistent PLAN behavior
across all HEE workflows while preventing the accidental mutations that have historically caused process failures.
